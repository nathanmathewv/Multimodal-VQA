...
def ensure_model_available(target_dir, kagglehub_link):
    if os.path.isdir(target_dir) and os.listdir(target_dir):
        print(f"Model found in '{target_dir}', skipping download.")
        return target_dir
    
    print("Fetching model weights from KaggleHub...")
    downloaded_path = kagglehub.model_download(kagglehub_link)

    # Move or link files to target_dir for consistency
...
if __name__ == "__main__":
    ...
    kagglehub_link = "deathlymade3/blip-vqa-lora-combined/pyTorch/default"
    model_dir = "./finetuned_blip_with_lora"
    run_inference(args.image_dir, args.csv_path, model_dir, kagglehub_link)